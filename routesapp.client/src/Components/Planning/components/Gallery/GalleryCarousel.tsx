...
import useAutoCarousel from "../../hooks/useAutoCarousel.ts";
...
function GalleryCarousel({ country, images, loading }: GalleryCarouselProps) {
    ...
    }, [activeIdx, transitioning, goTo]);

    const handlePrev = useCallback(() => {
        const i = activeIdx === 0 ? images.length - 1 : activeIdx - 1;
        navigateTo(i);
    }, [activeIdx, images.length, navigateTo]);

    const handleNext = useCallback(() => {
        const i = activeIdx === images.length - 1 ? 0 : activeIdx + 1;
        navigateTo(i);
    }, [activeIdx, images.length, navigateTo]);
    ...
    if (loading) {
        return (
            <div className="gallery-loading">
                ...
                <div className="gallery-loading__text">Cargando galería…</div>
            </div>
        );
    }
        ...
        <div className="gallery-root">
            <div className="gallery-hero" ref={heroRef}>
                ...
                <div className="gallery-chip">
                    <img src={country.flags.svg} alt={country.name.common} className="gallery-chip__flag" />
                    <span className="gallery-chip__name">{country.name.common}</span>
                    <span className="gallery-chip__badge">Galería</span>
                </div>

                <div className="gallery-counter">
                    <span className="gallery-counter__current">{String(activeIdx + 1).padStart(2, "0")}</span>
                    <span className="gallery-counter__sep">/</span>
                    <span className="gallery-counter__total">{String(images.length).padStart(2, "0")}</span>
                </div>

                <div className="gallery-caption">
                    <p className="gallery-caption__label">
                        {images[activeIdx]?.label?.replace(/^File:|\.jpg|\.jpeg|\.png/gi, "").trim()}
                    </p>
                ...
                <button className="gallery-nav gallery-nav--prev" onClick={handlePrev} aria-label="Anterior">←</button>
                <button className="gallery-nav gallery-nav--next" onClick={handleNext} aria-label="Siguiente">→</button>
            </div>

            <div className="gallery-strip">
                <div className="gallery-strip__track" ref={stripRef}>
                    {images.map((img, idx) => (
                        <button
                            key={img.id}
                            className={`gallery-strip__thumb ${idx === activeIdx ? "is-active" : ""}`}
                            onClick={() => navigateTo(idx)}
                            aria-label={`Ver imagen ${idx + 1}`}
                        >
                            <img src={img.url} alt="" />
                            <div className="gallery-strip__thumb-overlay" />
                            {idx === activeIdx && <div className="gallery-strip__thumb-bar" />}
                        </button>
                    ))}
                </div>
            </div>
